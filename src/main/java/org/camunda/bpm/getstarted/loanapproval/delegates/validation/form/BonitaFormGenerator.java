...
public class BonitaFormGenerator {
    ...
    public static int generateFormsFromConfigAndBpmn(
        ...
        Document procDocument = loadProcDocument(outputDir);
        if (procDocument == null) {
            System.out.println("No proc document found for Bonita form generation.");
            return 0;
        }
    ...
    private static Document loadProcDocument(String outputDir) {
        ...
        try (Stream<Path> paths = Files.list(outputPath)) {
            procFile = paths.filter(path -> path.getFileName().toString().endsWith(".proc"))
                    .findFirst()
                    .orElse(null);
        } catch (Exception e) {
            throw new RuntimeException("Failed to scan Bonita output directory for .proc file.", e);
        }
        ...
        try {
            ...
            return builder.parse(procFile.toFile());
        } catch (Exception e) {
            throw new RuntimeException("Failed to parse proc document: " + procFile, e);
        }
    }
