package generator.bpmn;

import generator.bpmn.models.*;
import generator.processConfig.models.Lane;
import generator.processConfig.models.Node;
import generator.bpmn.models.Collaboration;
import generator.bpmn.models.FlowNode;
import generator.bpmn.models.LaneSet;
import generator.bpmn.models.ProcessDef;
import generator.organization.Actor;
...
import generator.processConfig.models.Variable;
...
public class BpmnHelper {
     ...
     */
    public static List<DataObjectReference> getFlowNodeInputs(
            FlowNode flowNode,
            ProcessDef process) {

        List<DataObjectReference> inputs = new ArrayList<>();

        for (DataInputAssociation inputAssoc : flowNode
                .dataInputAssociations()) {
            if (inputAssoc.sourceRef() != null) {
                DataObjectReference dataObj = process
     ...
     */
    public static List<DataObjectReference> getFlowNodeOutputs(
            FlowNode flowNode,
            ProcessDef process) {

        List<DataObjectReference> outputs = new ArrayList<>();

        for (DataOutputAssociation outputAssoc : flowNode
    ...
    private static ProcessDef parseProcess(Element processEl) {
        ...
        Map<String, DataObjectReference> dataObjectsById = parseDataObjectReferences(
                processEl);
        ...
        Map<String, SequenceFlow> flowsById = parseSequenceFlows(processEl);
    ...
    private static LaneSet parseLaneSet(Element laneSetEl) {
        ...
        // Parse all lanes
        List<generator.bpmn.models.Lane> lanes = new ArrayList<>();
        ...
        for (int i = 0; i < laneNodes.getLength(); i++) {
            ...
            generator.bpmn.models.Lane lane = parseLane(laneEl);
            lanes.add(lane);
        }
     ...
     */
    private static generator.bpmn.models.Lane parseLane(Element laneEl) {
        ...
        return new generator.bpmn.models.Lane(id, name, flowNodeRefs);
    }

    /**
     * Parses data object references from a process element.
     */
    private static Map<String, DataObjectReference> parseDataObjectReferences(
            Element processEl) {
        Map<String, DataObjectReference> dataObjectsById = new HashMap<>();
        ...
        for (int i = 0; i < dataObjectRefNodes.getLength(); i++) {
            ...
            if (id != null && !id.isEmpty()) {
                ...
                DataObjectReference dataObjRef = DataObjectReference
        ...
        for (String nodeType : flowNodeTypes) {
            ...
            for (int i = 0; i < nodes.getLength(); i++) {
                ...
                if (nodeId != null && !nodeId.isEmpty()) {
                    ...
                    List<DataInputAssociation> dataInputs = parseDataInputAssociations(
                            nodeEl);

                    // Parse data output associations
                    List<DataOutputAssociation> dataOutputs = parseDataOutputAssociations(
                            nodeEl);
     ...
     */
    private static Map<String, SequenceFlow> parseSequenceFlows(Element processEl) {
     ...
     */
    private static List<DataInputAssociation> parseDataInputAssociations(
            Element nodeEl) {
        List<DataInputAssociation> dataInputs = new ArrayList<>();
     ...
     */
    private static List<DataOutputAssociation> parseDataOutputAssociations(
            Element nodeEl) {
        List<DataOutputAssociation> dataOutputs = new ArrayList<>();
        ...
        for (int i = 0; i < dataOutputNodes.getLength(); i++) {
            ...
            if (id != null && !id.isEmpty()) {
                DataOutputAssociation dataOutput = new DataOutputAssociation(
                        id, targetRef);
    ...
    public static BpmnData updateLanesWithAssignees(
        ...
        for (ProcessDef bpmnProcess : bpmnDefinitions.processes()) {
            ...
            if (bpmnProcess.laneSet() != null) {
                List<generator.bpmn.models.Lane> updatedLanes = new ArrayList<>();

                for (generator.bpmn.models.Lane bpmnLane : bpmnProcess.laneSet().lanes()) {
                    // Find matching config lane by name
                    Lane configLane = configProcess.config.lanes
                            .stream()
                    ...
                    generator.bpmn.models.Lane updatedLane = new generator.bpmn.models.Lane(
                            bpmnLane.id(),
                            bpmnLane.name(),
    ...
    public static BpmnData updateUserTasksWithLaneAssignees(BpmnData bpmnDefinitions) {
        ...
        for (ProcessDef process : bpmnDefinitions.processes()) {
            ...
            for (generator.bpmn.models.Lane lane : process.laneSet().lanes()) {
    ...
    public static BpmnData updateUserTasksWithFormRefsAndResolvedFormOutputVariableName(
        ...
        for (ProcessDef process : bpmnDefinitions.processes()) {
            ...
            // FlowNode
            for (Node configTask : configProcess.config.tasks) {
    ...
    public static BpmnData updateFlowNodesWithVendorAttributes(
        ...
        for (ProcessDef process : bpmnDefinitions.processes()) {
            ...
            if (configProcess.config.events != null) {
                for (Node configEvent : configProcess.config.events) {
    ...
    public static BpmnData resolveGlobalVariablesInSequenceFlows(
        ...
        for (ProcessDef process : bpmnDefinitions.processes()) {
            ...
            Map<String, SequenceFlow> updatedFlows = new HashMap<>();

            for (Map.Entry<String, SequenceFlow> entry : process.flowsById().entrySet()) {
                SequenceFlow flow = entry.getValue();
